...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    attempts = []
    def get_money(withdraw, input):
        nonlocal attempts, password, balance
        if len(attempts)==3:
...
class VendingMachine:
    ...
    def deposit(self, val):
        if self.stock==0:
            return 'Machine is out of stock. Here is your ${}.'.format(val)
        else:
            self.balance+=val
    ...
    def vend(self):
        ...
        elif curr_bal<self.price:
            return 'You must deposit ${} more.'.format((self.price - self.balance))
        self.balance = 0
        self.stock-=1
        if curr_bal==self.price:
            return 'Here is your {}.'.format(self.product)
        else:

            return 'Here is your {} and ${} change.'.format(self.product, curr_bal-self.price)
    def restock(self, val):
        self.stock+=val
        return 'Current {} stock: {}'.format(self.product, str(self.stock))
