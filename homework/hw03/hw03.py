...
def replace_leaf(t, old, new):
    ...
    new_branches = []
    for b in branches(t):
      new_branches.append(replace_leaf(b, old, new))
...
def balanced(m):
    ...
    def get_torque(s):
      if is_weight(end(s)):
        ...
        if get_torque(left(end(s)))==get_torque(right(end(s))):
          return total_weight(end(s))*length(s)
    return get_torque(left(m)) == get_torque(right(m))
...
def totals_tree(m):
    ...
    l_e = end(left(m))
    r_e = end(right(m))
    left_tree = tree(total_weight(l_e))
    right_tree = tree(total_weight(r_e))
    if is_weight(l_e) and is_weight(r_e):
      return tree(total_weight(m), [left_tree, right_tree])
    elif is_mobile(l_e) and is_weight(r_e):
      return tree(total_weight(m), [totals_tree(l_e), right_tree])
    elif is_mobile(r_e) and is_weight(l_e):
      return tree(total_weight(m), [left_tree, totals_tree(r_e)])
    else:
      return tree(total_weight(m), [totals_tree(l_e), totals_tree(r_e)])
