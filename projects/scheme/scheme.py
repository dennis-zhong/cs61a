...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(procedure, rest.map(lambda operand: scheme_eval(operand, env)), env)
        # END PROBLEM 5
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals)<len(vals) or len(formals)>len(vals):
            raise SchemeError
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target, scheme_eval(second.first, env))
        return target
        # END PROBLEM 6
        ...
        "*** YOUR CODE HERE ***"
        name = target.first
        lambdapart = Pair(target.second, expressions.second)
        env.define(name, do_lambda_form(lambdapart, env))
        return name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        if expressions.second == nil:
            expression = scheme_eval(expressions.first, env, True)
        else:
...
def make_let_frame(bindings, env):
    ...
    for _ in range(len(bindings)):
        cur_bind = bindings.first
        check_form(cur_bind, 2, 2)
        formals = Pair(cur_bind.first, formals)
        vals = Pair(scheme_eval(cur_bind.second.first, env), vals)
