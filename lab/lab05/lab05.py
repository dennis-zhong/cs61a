...
def square_tree(t):
    if is_leaf(t):
        ...
        return tree(label(t)**2, square_tree(branches(t)))
...
def replace_with_sum(t):
    ...
    branch_sum = sum([label(t)]+[label(b) for b in new_branches])
    return tree(branch_sum, new_branches)

def t_to_lst(t):
    lst = [label(t)]
    for b in branches(t):
        lst.append(t_to_lst(b))
    return lst